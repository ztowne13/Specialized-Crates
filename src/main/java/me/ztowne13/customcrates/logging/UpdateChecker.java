...
public class UpdateChecker
{
		...
		Bukkit.getScheduler().runTaskAsynchronously(cc, new Runnable()
		{
			...
			public void run()
			{
				try
				{
					...
					con.getOutputStream().write(("key=98BE0FE67F88AB82B4C197FAF1DC3B69206EFDCC4D3B80FC83A00037510B99B4&resource=" + 9047).getBytes("UTF-8"));
	...
	public void updateNeedsUpdate()
	{
		...
		for(int i = 0; i < 4; i++)
		{
			...
			{
				ChatUtils.log(new String[]{"There is an update for Specialized Crates! You are currently on version " + thisVersion + " but an update for version " + latestVersion + " is available."});
				notifyPlayers();
				needsUpdate = true;
				return;
			}

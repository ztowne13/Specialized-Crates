...
public class IGCCrateFireworks extends IGCTierMenu
{
	...
	public void open()
	{
		...
		InventoryBuilder ib = createDefault(InventoryUtils.getRowsFor(4, (crates.getCs().getCf().getFireworks().containsKey(tier) ? cs.getCf().getFireworks().get(tier).size() : 0)) + 9);
		...
		if(cs.getCf().getFireworks().containsKey(tier))
		{
			...
			for (FireworkData fd : cs.getCf().getFireworks().get(tier))
			{
				...
				ib.setItem(i, new ItemBuilder(Material.NETHER_STAR, 1, 0).setName("&aColors: &7" + fd.getColors()).setLore("&aFade Colors: &7" + fd.getFadeColors()).addLore("&aPower: &7" + fd.getPower()).addLore("&aFlicker: &7" + fd.isFlicker()).addLore("&aTrail: &7" + fd.isTrail()).addLore("&aType: &7" + fd.getFeType().name()));
				i++;
			}
		}
	...
	public void manageClick(int slot)
	{
		...
		else if(getIb().getInv().getItem(slot) != null && getIb().getInv().getItem(slot).getType().equals(DynamicMaterial.FIREWORK_ROCKET.parseMaterial()))
		{
			if(deleteMode)
			{
				cs.getCf().removeFireworks(tier, cs.getCf().getByString(tier, ChatUtils.removeColor(getIb().getInv().getItem(slot).getItemMeta().getDisplayName())));
				open();
			}
		}
	}

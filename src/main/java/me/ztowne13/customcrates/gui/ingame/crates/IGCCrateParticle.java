...
public class IGCCrateParticle extends IGCTierMenu
{
	...
	public void open()
	{
		...
		ib.setItem(12, new ItemBuilder(Material.BLAZE_ROD, 1, 0).setName("&aX Offset").setLore("Distance particles will spawn").addLore("in the x direction.").addLore("").addLore("&7Current value: ").addLore("&7" + pd.getOffX()));
		ib.setItem(13, new ItemBuilder(Material.BLAZE_ROD, 1, 0).setName("&aY Offset").setLore("Distance particles will spawn").addLore("in the y direction.").addLore("").addLore("&7Current value: ").addLore("&7" + pd.getOffY()));
		ib.setItem(14, new ItemBuilder(Material.BLAZE_ROD, 1, 0).setName("&aZ Offset").setLore("Distance particles will spawn").addLore("in the z direction.").addLore("").addLore("&7Current value: ").addLore("&7" + pd.getOffZ()));
		ib.setItem(15, new ItemBuilder(Material.SUGAR, 1, 0).setName("&aParticle speed").setLore("&7Current value: ").addLore("&7" + pd.getSpeed()));
		ib.setItem(16, new ItemBuilder(Material.BUCKET, 1, 0).setName("&aParticle amount").setLore("&7Current value: ").addLore("&7" + pd.getAmount()));
	...
	public void manageClick(int slot)
	{
		switch(slot)
		{
			...
			case 0:
				...
				break;
			case 10:
				new InputMenu(getCc(), getP(), "particle animation", pd.getParticleName(), "Type 'NONE' to remove animations\nAvaialable animations: " + Arrays.toString(PEAnimationType.values()), String.class, this);
				break;
			case 11:
				new InputMenu(getCc(), getP(), "particle type", pd.getParticleName(), "Avaialable particles: " + Arrays.toString(ParticleEffect.values()), String.class, this);
				break;
			case 12:
				new InputMenu(getCc(), getP(), "x offset", pd.getOffX()+"", "Distance particles will spawn in the x direction relative to the crate.", Double.class, this);
				break;
			case 13:
				new InputMenu(getCc(), getP(), "y offset", pd.getOffY()+"", "Distance particles will spawn in the y direction relative to the crate.", Double.class, this);
				break;
			case 14:
				new InputMenu(getCc(), getP(), "z offset", pd.getOffZ()+"", "Distance particles will spawn in the z direction relative to the crate.", Double.class, this);
				break;
			case 15:
				new InputMenu(getCc(), getP(), "speed", pd.getSpeed()+"", "Changes the speed of most of the particles. For some, like music notes, it changes the color.", Double.class, this);
				break;
			case 16:
				new InputMenu(getCc(), getP(), "amount", pd.getAmount()+"", "How many particles spawn every tick (1/20th of a second).", Integer.class, this);
				break;
		}
	}
	...
	public boolean handleInput(String value, String input)
	{
		...
		else if(value.equalsIgnoreCase("particle animation"))
		{
			...
			{
				ChatUtils.msgError(getP(), input + " is not valid from the list of animations: " + Arrays.toString(PEAnimationType.values()));
				return false;
			}
		}
		return false;

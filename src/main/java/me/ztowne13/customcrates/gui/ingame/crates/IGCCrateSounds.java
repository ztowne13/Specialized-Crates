...
public class IGCCrateSounds extends IGCTierMenu
{
	...
	public void open()
	{
		...
		InventoryBuilder ib = createDefault(InventoryUtils.getRowsFor(4, (crates.getCs().getCs().getSounds().containsKey(tier) ? crates.getCs().getCs().getSounds().get(tier).size() : 0) + 9));

		ib.setItem(9, IGCDefaultItems.EXIT_BUTTON.getIb());
		ib.setItem(8, new ItemBuilder(Material.PAPER, 1, 0).setName("&aCreate a new sound").setLore("&7Make sure you save").addLore("&7when you're done."));
		...
		if(crates.getCs().getCs().getSounds().containsKey(tier))
		{
			...
			for (SoundData sd : crates.getCs().getCs().getSounds().get(tier))
			{
				...
				ib.setItem(i, new ItemBuilder(Material.NOTE_BLOCK, 1, 0).setName("&a" + sd.getSound().name()).setLore("&7Pitch: &f" + sd.getPitch()).addLore("&7Volume: &f" + sd.getVolume()));
				slots.put(i, sd);
				i++;
			}
		}
	...
	public void manageClick(int slot)
	{
		...
		{
			new InputMenu(getCc(), getP(), "sound type", "null", "Click for a list of sounds -> https://www.spigotmc.org/wiki/cc-sounds-list/", String.class, this);
		}
		else if(getIb().getInv().getItem(slot) != null && getIb().getInv().getItem(slot).getType().equals(Material.NOTE_BLOCK))
		{

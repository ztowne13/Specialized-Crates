...
public abstract class CrateHead 
{
	...
	public boolean playerPassesKeyTest(Player p, boolean requireKeyInHand)
	{
		...
		return !settings.isRequireKey() || (requireKeyInHand ? hasItemInHand(p, settings) : hasKeyInInventory(p, settings)) || PlayerManager.get(cc, p).getPdm().getVCCrateData(getCrates()).getKeys() > 0;
	}
	...
	public boolean hasItemInHand(Player p, CrateSettings settings)
	{
		try
		{
			return p.getItemInHand().getType().equals(settings.getKey(1).getType()) && p.getItemInHand().getItemMeta().getDisplayName().equalsIgnoreCase(settings.getKey(1).getItemMeta().getDisplayName()) && keysLoreMatches(p.getItemInHand(), settings);
		}
		catch(Exception exc)
		{
			return false;
	...
	public boolean hasKeyInInventory(Player p, CrateSettings settings)
	{
		for(ItemStack stack: p.getInventory().getContents())
		{
			try
			{
				if(stack.getType().equals(settings.getKey(1).getType()) && stack.getItemMeta().getDisplayName().equalsIgnoreCase(settings.getKey(1).getItemMeta().getDisplayName()) && keysLoreMatches(stack, settings))
				{
					return true;
				}
			}
	...
	public boolean canExecuteFor(CrateState current, CrateState cs, Player p, boolean requireKeyInHand)
	{
		...
		boolean hasPropperOpeningTools = (itemNotNull(p) && playerPassesKeyTest(p, requireKeyInHand)) || !getCrates().getCs().isRequireKey();
		boolean passesInventoryCheck = !PlayerManager.get(cc, p).isInCrate() || PlayerManager.get(cc, p).getOpenCrate().isMultiCrate();
				...
				for (int i = 0; i < 36; i++)
				{
					try
					{
						if (!(p.getInventory().getItem(i) == null))
						{
							...
							if (stack.getType().equals(getCrates().getCs().getKey(1).getType()) && stack.getItemMeta().getDisplayName().equalsIgnoreCase(getCrates().getCs().getKey(1).getItemMeta().getDisplayName()))
							{
	...
	public void completeCrateRun(Player p, ArrayList<Reward> rewards, boolean overrideAutoClose)
	{
		...
		new HistoryEvent(Utils.currentTimeParsed(), getCrates(), rewards, true).addTo(PlayerManager.get(getCc(), p).getPdm());

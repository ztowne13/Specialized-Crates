	...
	*/
	...
	public InventoryBuilder getInventory(Player p, String invName)
	{
		...
		for (int i : crateSpots.keySet())
		{
			try
			{
				...
				{
					crateIb.addLore(cc.getSettings().getConfigValues().get("virtual-crate-lore").toString().replaceAll("%crates%", vcd.getCrates() + ""));
				}
				if(cc.getSettings().getConfigValAsBoolean("virtual-crate-keycount"))
				{
					crateIb.addLore(cc.getSettings().getConfigValues().get("virtual-key-lore").toString().replaceAll("%keys%", vcd.getKeys() + ""));
				}
	...
	public void saveToFile()
	{
		...
		for(int i = 0; i < inv.getSize(); i++)
		{
			...
			if(CrateUtils.searchByCrate(stack) == null)
			{
				...
				{
					if(alreadyExisting.getType() == stack.getType() && alreadyExisting.getDurability() == stack.getDurability())
					{
						b = false;
						break;
					}
				...
				for(String s : materialsWithID.keySet())
				{
					...
					catch(Exception exc)
					{
						for(Player p : Bukkit.getOnlinePlayers())
						{
							if(p.isOp())
							{
								ChatUtils.msgError(p, "Failed to save multicrate " + stack.getType().name() + " with value " + s);
							}
						}
						exc.printStackTrace();
					}
	...
	public void checkClick(PlayerManager pm, int slot, ClickType clickType)
	{
		...
		if(crate.getCs().getCmci().getCrateSpots().keySet().contains(slot))
		{
			...
			if(clickType.equals(Boolean.valueOf(cc.getSettings().getConfigValues().get("mc-reward-display-leftclick").toString().toUpperCase()) ? ClickType.LEFT : ClickType.RIGHT) && (Boolean) SettingsValues.REWARD_DISPLAY_ENABLED.getValue(cc))
			{

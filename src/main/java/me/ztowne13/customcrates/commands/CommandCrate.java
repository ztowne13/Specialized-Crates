...
public class CommandCrate extends Commands implements CommandExecutor
{
	...
	{	
		...
		if(args.length >= 1 && args[0].equalsIgnoreCase("luckychest") && canExecute(false, true, "customcrates.luckychestcommand"))
		{
			PlayerDataManager pdm = PlayerManager.get(cc, (Player)sender).getPdm();
			pdm.setActivatedLuckyChests(!pdm.isActivatedLuckyChests());
			Messages.TOGGLE_LUCKYCRATE.msgSpecified(cc, (Player)sender, new String[]{"%state%"}, new String[]{pdm.isActivatedLuckyChests() + ""});
			return true;
		}
		else if(!canExecute(false, true,  "customcrates.admin"))
		{
			...
			msg(Messages.NO_PERMISSIONS.getFromConf(cc).replaceAll("%permission%", "customcrates.crates / customcrates.admin"));
		}
		else
		{
			msgPage(1);

...
public class NBTTagBuilder {
    private static final String[] excludedTags = new String[]{
            "display",
            "Enchantments",
            "ench",
            "HideFlags",
    ...
    private static final String[] booleanTags = new String[]{
            "Unbreakable"
    };
    ...
    public static ItemStack applyTo(ItemStack item, String tag) {
        ...
        try {
            ...
            StringBuilder remainingValue = new StringBuilder();
            for (int i = 1; i < args.length; i++) {
                remainingValue.append(args[i]).append(" ");
            }
            remainingValue = new StringBuilder(remainingValue.substring(0, remainingValue.length() - 1));

            value = remainingValue.toString().replace(",Properties:\\{textures:\\[0:\\{Value:", ",Properties:{textures:[{Value:");
        } catch (Exception exc) {
            exc.printStackTrace();

	...
	public void place(PlacedCrate cm)
	{
		...
		if(cm.getCrates().isEnabled())
		{
			if (!cm.getL().getBlock().getType().equals(cm.getCrates().getCs().getCrate(1).getType()))
			{
				...
				{
					StatusLoggerEvent.SETTINGS_CRATE_FAILURE_DISABLE.log(cm.getCrates().getCs().getSl(), new String[]{m.toString() + " is not a block and therefore cannot be used as a crate type!"});
